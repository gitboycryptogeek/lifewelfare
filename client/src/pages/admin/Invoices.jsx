  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (invoice) => {
      ...
      try {
        ...
        toast.error('Invoice saved but PDF download failed. Download from History.');
      } finally {
        setPdfLoading(null);
      }
      ...
      <div className="p-4 sm:p-6 max-w-5xl mx-auto">
        ...
        {tab === 'generate' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Live preview card */}
            <div className="bg-brand-navy rounded-xl p-6 text-white flex flex-col justify-between min-h-[340px]">
              <div>
                <p className="text-brand-gold font-heading font-bold text-lg mb-1">My Life Companion Welfare</p>
                <p className="text-xs text-gray-400">Development House, Floor 13, Suite 18</p>
                <p className="text-xs text-gray-400 mb-6">Underwritten by Old Mutual</p>
                ...
                {selectedPlan && (
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-300">Annual Premium (Opt {selectedPlan.option})</span>
                      <span>{fmtKES(planAmount)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-300">Cover amount</span>
                      <span className="text-green-400">{selectedPlan.cover}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-300">Joining Fee</span>
                      <span>{fmtKES(memberFee)}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="mt-3 bg-brand-gold/20 rounded-lg p-3 text-xs text-gray-300">
                  Pay via M-Pesa Paybill <span className="text-brand-gold font-semibold">625625</span>
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {invoices.map((inv) => (
                          <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-4 py-3 font-mono text-xs text-brand-navy font-semibold">
                              {inv.invoice_number}
                            </td>
                            <td className="px-4 py-3 font-medium text-gray-800">{inv.client_name}</td>
                            <td className="px-4 py-3 text-gray-500 hidden md:table-cell">
                              Option {inv.cover_option}
                            </td>
                            ...
                            <td className="px-4 py-3 text-gray-500 text-xs hidden lg:table-cell">
                              {inv.created_by_name}
                              <br />
                              <span className="capitalize">{inv.created_by_role?.replace('_', ' ')}</span>
                            </td>
